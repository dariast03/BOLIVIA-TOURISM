<script lang="ts">
	...
	$: filtroDatos = blogs.filter(
		(item) =>
			item.titulo.toLocaleLowerCase().includes(textoBuscar.toLocaleLowerCase()) ||
			item.autor.toLocaleLowerCase().includes(textoBuscar.toLocaleLowerCase()) ||
...
<TableSearch placeholder="Busca algo...." hoverable={true} bind:inputValue={textoBuscar}>
	<TableHead>
		...
		<TableHeadCell>Vistas</TableHeadCell>
		<TableHeadCell>Acciones</TableHeadCell>
	</TableHead>

	<TableBody>
		{#each filtroDatos as blog}
			<TableBodyRow>
				<TableBodyCell>{blog.id}</TableBodyCell>
				<TableBodyCell>{blog.titulo}</TableBodyCell>
