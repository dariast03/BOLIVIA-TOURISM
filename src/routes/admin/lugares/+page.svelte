<script lang="ts">
	...
	const getData = async () => {
		const response = await fetch(
			PUBLIC_BASE_URL + `/lugares?_start=${limite - 20}&_end=${limite}`,
			{
				method: 'GET',
				headers: {
	...
	onMount(async () => {
		...
		const target = document.querySelector('#ultimoRegistro');
		if (target) {
...
<div class="flex justify-between">
	<Heading>LUGARES TURISTICOS</Heading>
...
<TableSearch placeholder="Busca algo...." hoverable={true} bind:inputValue={textoBuscar}>
	...
	<TableBody>
		{#each filtroDatos as lugar, i}
			<TableBodyRow>
				<TableBodyCell>{lugar.id || -1}</TableBodyCell>
				<TableBodyCell tdClass="max-w-[100px]">{lugar.nombre}</TableBodyCell>
				<!-- 	<TableBodyCell>{lugar.descripcion}</TableBodyCell> -->
				<!-- <TableBodyCell>{lugar.ubicacion.latitud}, {lugar.ubicacion.longitud}</TableBodyCell> -->
				<TableBodyCell>{lugar.departamento}</TableBodyCell>
				<TableBodyCell>{lugar.votos}</TableBodyCell>
				<TableBodyCell>
					<Button color="yellow">
						<svg
	...
	<div id="ultimoRegistro" class="text-transparent">FIN DATA</div>
</TableSearch>
